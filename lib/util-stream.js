...
export class CheckLicenseTypeTransform extends Transform {
  ...
  _transform(chunk, _encoding, callback) {
    ...
    const licenseType = currentPackage.licenseType?.trim() ?? '';
    let licenseTypes = [];
    ...
    if (licenseType.includes(' ')) {
      if (this._isKnownLicenseExpression(licenseType)) {
        licenseTypes = licenseType.split(/(?:^\(|\)$|\sOR\s)/).filter((word) => word.length > 0);
      } else {
    ...
    if (this.allowed.length > 0 && !this._arrayIncludesAny(this.allowed, licenseTypes)) {
      this.result.notAllowed.push(currentPackage);
    }

    // all license types must be forbidden for a package to be forbidden
    if (this.forbidden.length > 0 && this._arrayIncludesAll(this.forbidden, licenseTypes)) {
  ...
  _arrayIncludesAny(allowedValues, valuesToValidate) {
    return valuesToValidate.some((inspectedValue) => allowedValues.includes(inspectedValue));
  }
  ...
  _arrayIncludesAll(allowedValues, valuesToValidate) {
    return valuesToValidate.every((inspectedValue) => allowedValues.includes(inspectedValue));
  }
}
...
export class FormatterWritable extends Writable {
  ...
  _write(chunk, _encoding, callback) {
    // Write formatted result to stdout
    console.log(this.formatter(chunk, this.config));

    // Set exit code
    if (chunk.notAllowed.length !== 0) {
      process.exitCode = process.exitCode ? process.exitCode | 2 : 2;
    }

    if (chunk.forbidden.length !== 0) {
      process.exitCode = process.exitCode ? process.exitCode | 4 : 4;
    }

    if (chunk.unknown.length !== 0) {
      process.exitCode = process.exitCode ? process.exitCode | 8 : 8;
    }
